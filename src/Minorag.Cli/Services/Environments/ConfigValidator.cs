    ...
    string? configuredDbPath) : IValidator
{
    ...
    public async IAsyncEnumerable<EnvironmentCheckResult> ValidateAsync([EnumeratorCancellation] CancellationToken ct)
    {
        ...
        {
            yield return new("Configured Database", $"Path: [cyan]{Markup.Escape(configuredDbPath)}[/]", EnvironmentIssueSeverity.Success);
        }
        ...
        {
            var directory = Markup.Escape(fileInfo.DirectoryName ?? "(unknown)");
        ...
        else if (fileInfo.Exists)
        {
            ...
            {
                yield return new("DB directory", $"Could not verify write permissions for DB directory: [yellow]{Markup.Escape(message ?? "")}[/]", EnvironmentIssueSeverity.Error);
            }
        }
        else
        {
        ...
        EnvironmentCheckResult? CheckEnvOverride(string envName, string? configValue)
        {
            ...
            if (hasOverwrite)
            {
                return new EnvironmentCheckResult("Environment override", $"[cyan]{envName}[/] = [blue]{Markup.Escape(envValue ?? "(null)")}[/] differs from config value ([blue]{Markup.Escape(configValue ?? "(null)")}[/]).", EnvironmentIssueSeverity.Warning);
            }
