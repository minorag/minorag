...
public class OllamaValidator(IOllamaClient client, IOptions<OllamaOptions> ollamaOptions) : IValidator
{
    ...
    private static EnvironmentCheckResult CheckModelInstalled(HashSet<string> models, string? model, string label)
    {
        ...
        {
            return new EnvironmentCheckResult(Label, $"{label} [cyan]{modelEscaped} installed [/]", EnvironmentIssueSeverity.Success);
        }

        return new EnvironmentCheckResult(Label, $"Model [cyan]{modelEscaped}[/] is not installed. Run: [cyan]`ollama pull {modelEscaped}`[/]", EnvironmentIssueSeverity.Warning);
    }
    ...
    private async Task<(HashSet<string>, EnvironmentCheckResult)> GetTags(CancellationToken ct)
    {
        ...
        return (modelNames, new EnvironmentCheckResult(Label, $" Ollama reachable at [cyan]{Markup.Escape(client.Host)}[/].", EnvironmentIssueSeverity.Success));
    }
}
